...
internal object ComponentTypeIDsResolver {
    ...
    internal val maxIndex: Int

    init {
        val allComponentClasses = ReflectionUtils.getAllComponentClasses()
        val functionArguments = generateFunctionArguments(allComponentClasses)
        maxIndex = functionArguments.maxIndex
    ...
    private fun <T> generateFunctionArguments(components: Collection<T>): FunctionArguments {
    ...
    class FunctionArguments(val divider: Int, val modulo: Int, val hasModulo: Boolean, val maxIndex: Int)
